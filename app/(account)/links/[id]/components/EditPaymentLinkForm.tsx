...
import { updatePaymentLink } from '@/actions/paymentLinks/updatePaymentLink'
import { Loader2 } from 'lucide-react'
import { type PaymentLinkDetail } from '@/actions/paymentLinks/getPaymentLink'

const formSchema = z.object({
  title: z.string().min(1, 'Title is required').max(100, 'Title cannot exceed 100 characters'),
  slug: z.string().min(3, 'Slug must be at least 3 characters').max(50, 'Slug cannot exceed 50 characters').regex(/^[a-z0-9-]+$/, 'Slug can only contain lowercase letters, numbers, and hyphens'),
...
export function EditPaymentLinkForm({ link }: Props) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      amountType: link.amountType as 'fixed' | 'flexible',
      amount: link.amount?.toString() || '',
      minAmount: link.minAmount?.toString() || '1',
      maxAmount: link.maxAmount?.toString() || '',
      currency: link.currency as 'KES' | 'USD' | 'EUR' | 'GBP',
      hasUsageLimit: link.hasUsageLimit,
      usageLimit: link.usageLimit?.toString() || '',
  ...
  async function onSubmit(data: FormValues) {
    ...
    const payload = {
      ...
      usageLimit: data.hasUsageLimit && data.usageLimit ? parseInt(data.usageLimit) : null,
      expiresAt: data.hasExpiry && data.expiresAt ? data.expiresAt : null,
